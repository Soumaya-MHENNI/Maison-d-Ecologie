...
//					},800);
		...
		/*addOrRenameOnEnterClicked: function(){
			'use strict';
			var activeLi = jQuery('ul.cc_categorify_category_list li.rename');
			activeLi.find('.cc_changer input').on("keyup", function(event){
				event.preventDefault();
				
				console.log(activeLi.find('.cc_btns .cc_apply').length);
				
				if (event.keyCode === 13 && self.stopAction === 0) {
					activeLi.find('.cc_btns .cc_apply').trigger('click');
					self.stopAction = 1;
				}
			});
		},
		*/
		...
		tripleActionsOnCategories: function(currentItem){
			...
			jQuery('ul.cc_categorify_category_list .cc_changer,ul.cc_categorify_category_list .cc_btns').on('click',function(e){
				e.preventDefault();
				e.stopPropagation();
