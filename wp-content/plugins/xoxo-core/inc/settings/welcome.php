...
$max	= 6;
$URL	= XOXO_PLUGIN_URL;
$ver	= XOXO_PLUGIN_VERSION;
$site	= 'https://frenify.com/';
$ttext	= XOXO_TEXT_DOMAIN;
...
if( !function_exists('frenify__shuffle_assoc') ){
	function frenify__shuffle_assoc($list,$important = array()) { 
		if (!is_array($list)){ return $list; }
		$keys = array_keys($list);
		shuffle($keys);
		$random = array();
		if(!empty($important) && isset($important) && $important !== ''){
			foreach($important as $key){
				$random[$key] = $list[$key];
			}
		}
		$keys = array_diff($keys, $important);
		foreach ($keys as $key) { 
			$random[$key] = $list[$key]; 
		}
		return $random; 
...
$list 	= [
	...
	'waxon' => [
		'title'	=> 'Waxon',
		'name'	=> 'Personal & Portfolio WordPress Theme',
		'url'	=> 'https://themeforest.net/item/waxon-personal-portfolio-wordpress-theme/35292883',
		'image'	=> $URL.'inc/settings/assets/img/item-waxon.jpg'
	],
	'xoxo-core' => [
		'title'	=> 'Xoxo',
		'name'	=> 'NFT Portfolio WordPress Theme',
		'url'	=> 'https://themeforest.net/item/xoxo-nft-portfolio-wordpress-theme/39859785',
		'image'	=> $URL.'inc/settings/assets/img/item-waxon.jpg'
	],
];
$important = ['waxon'];
...
<div class="frenify__welcome">
	...
	<?php if($active != 'categorify' && !in_array('waxon',$important)){ ?>
	...
	<div class="frenify__welcome_list">
		<ul>
			<?php 
				...
				// shuffle array
				$list 		= frenify__shuffle_assoc($list,$important);
