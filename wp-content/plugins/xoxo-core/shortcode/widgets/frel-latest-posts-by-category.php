 ...
 * Frel Site Title
 */
class Frel_Latest_Posts_By_Category extends Widget_Base {

	public function get_name() {
		return 'frel-latest-posts-by-category';
	}

	public function get_title() {
		return __( 'Latest Posts by Category', 'xoxo-core' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => esc_html__( 'What\'s new?', 'xoxo-core' ),
	...
	protected function render() {
		...
		if($cat_list == 'all'){
			...
			if(!empty($categories)){
				...
				foreach($categories as $slug => $catName){
					...
					$categoryList .= '<li><a href="#" data-count="'.$post_number.'" data-layout="triple_post" data-id="'.$slug.'" class="'.$selected.'" data-link="'.get_term_link( $slug, 'category' ).'"><span><span class="text">'.$catName.'</span><span class="suffix">//</span></span></a></li>';
				}
			}		
		}else{
		...
		if(isset($activeCategory)){
			...
			$postList = xoxo_fn_cs_posts_triple($slug,$post_number);
			


			$html .= '<div class="fn_cs_posts_triple fn_cs_posts__ajax">';
